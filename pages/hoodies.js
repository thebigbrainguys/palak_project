      ...
      <section className="text-gray-600 body-font">
        <div className="container py-5 mx-auto">
          <div className="flex flex-wrap m-8 text-center justify-center">
            {Object.keys(products).length == 0 && <p>Hoodies are currently out of stock. More stock comming soon. Stay Tuned!</p>}
  ...
  let hoodies = {}
    for(let item of products){
        if(item.title in hoodies){
            if(!hoodies[item.title].colour.includes(item.colour) && item.availableQuantity > 0){
                hoodies[item.title].colour.push(item.colour)
            }
            if(!hoodies[item.title].size.includes(item.size) && item.availableQuantity > 0){
