      ...
      <section className="text-gray-600 body-font">
        <div className="container py-5 mx-auto">
          <div className="flex flex-wrap m-8 text-center justify-center">
            {Object.keys(products).length == 0 && <p className='flex items-center justify-center h-[50vh]'>Shoes are currently out of stock. More stock comming soon. Stay Tuned!</p>}
              ...
              return(
                  <div key={products[item].slug} className="lg:w-1/5 md:w-1/2 p-4 w-full shadow-md m-6">
                    <Link href={`/product/${products[item].slug}`}>
                      ...
                      <div className="mt-4">
                        <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">Mug</h3>
  ...
  let shoes = {}
    for(let item of products){
        if(item.title in shoes){
            if(!shoes[item.title].colour.includes(item.colour) && item.availableQuantity > 0){
                shoes[item.title].colour.push(item.colour)
            }
            if(!shoes[item.title].size.includes(item.size) && item.availableQuantity > 0){
                shoes[item.title].size.push(item.size)
            }
        }else{
            shoes[item.title] = JSON.parse(JSON.stringify(item));
            if(item.availableQuantity>0){
                shoes[item.title].colour  = [item.colour]
